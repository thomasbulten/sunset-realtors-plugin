...
namespace BP\Plugin\Setup\Updater;
 ...
 */
function plugin_info($res, string $action, object $args)
{ //phpcs:ignore
    ...
    if (BP_PLUGIN_SLUG !== $args->slug) {
        return $res;
    }

    $remote = get_transient('bpp_upgrade_' . BP_PLUGIN_SLUG);

    if (false === $remote) {

        $remote = wp_remote_get(
            BP_PLUGIN_UPDATE_URL . '/get-info.php?slug=' . BP_PLUGIN_SLUG . '&action=info',
            [
                'timeout' => 5,
                'headers' => [
        ...
        if (! is_wp_error($remote) && isset($remote['response']['code']) && 200 === $remote['response']['code'] && ! empty($remote['body'])) {
            set_transient('bpp_upgrade_' . BP_PLUGIN_SLUG, $remote, 21600); // 6 hours cache.
        }
    }
...
{ //phpcs:ignore
    ...
    if (false === $remote) {
        ...
        $remote = wp_remote_get(
            BP_PLUGIN_UPDATE_URL . '/get-info.php?slug=' . BP_PLUGIN_SLUG . '&action=update',
            [
                'timeout' => 10,
                'headers' => [
                    'Accept' => 'application/json',
    ...
    if (! isset($remote->error)) {
        ...
        if ($remote && version_compare(BP_PLUGIN_VERSION, $remote->version, '<') && version_compare($remote->requires, get_bloginfo('version'), '<')) {
            $res       = new \stdClass();
            $res->slug = BP_PLUGIN_SLUG;

            // it could be just mypluginslug1.php if your plugin doesn't have its own directory (my does).
            $res->plugin                         = BP_PLUGIN_SLUG . '/' . BP_PLUGIN_SLUG . '.php';
...
function plugin_after_update(\WP_Upgrader $upgrader_object, array $options): void
{
    if ('update' === $options['action'] && 'plugin' === $options['type']) {
        ...
        delete_transient('bpp_upgrade_' . BP_PLUGIN_SLUG);
    }
}
